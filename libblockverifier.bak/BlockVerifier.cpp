...
ExecutiveContext::Ptr BlockVerifier::executeBlock(Block& block, BlockInfo const& parentBlockInfo)
{
    ...
    try
    {
        ...
        context = serialExecuteBlock(block, parentBlockInfo); // 统一串行执行
    ...
    Block& block, BlockInfo const& parentBlockInfo)
{
    ...
    try
    {
        ...
        for (size_t i = 0; i < block.transactions()->size(); i++)
        {
            ...
            /*
                B1. 在逐个执行区块中的交易时，判断当前交易是否为跨片交易
            */

                // bytes txrlp = tx->rlp();
                // std::string txrlp_str = toHex(txrlp);
                // BLOCKVERIFIER_LOG(INFO) << LOG_DESC("执行模块处理的交易RLP编码为:")
                //                         << LOG_KV("txrlp_str", txrlp_str);

            BLOCKVERIFIER_LOG(INFO) << LOG_DESC("开始执行交易----------------");
